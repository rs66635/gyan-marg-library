...
public class WebController {
	...
	@PostMapping("/admin/member/save-new") // Changed endpoint to avoid conflict
	public String saveMember(@ModelAttribute Member member, Model model, HttpSession session) {
		...
		model.addAttribute("whatsappMessage", "Sent to " + saved.getMobileNumber() + ": Your member ID is "
				+ saved.getId() + " and password is " + saved.getPassword());
	...
	public String searchMember(@RequestParam(required = false) String q, Model model, HttpSession session) {
		...
		if (q == null || q.isBlank()) {
			...
			try {
				...
				memberService.findById(id).ifPresentOrElse(m -> model.addAttribute("members", List.of(m)),
						() -> model.addAttribute("members", List.of()));
			} catch (NumberFormatException e) {
				// by name or mobile
				List<Member> byName = memberService.searchByName(q);
				if (!byName.isEmpty())
					model.addAttribute("members", byName);
				else
					memberService.findByMobile(q).ifPresentOrElse(m -> model.addAttribute("members", List.of(m)),
							() -> model.addAttribute("members", List.of()));
			}
		}
		return "admin-dashboard";
	...
	public String memberDoLogin(@RequestParam Long memberId, @RequestParam String password, HttpSession session,
			Model model) {
		model.addAttribute("hideAdminLogin", true);
	...
	@PostMapping("/member/book-web") // Renamed endpoint to avoid conflict
	...
	@PostMapping("/member/logout-web") // Renamed endpoint to avoid conflict
	...
	@GetMapping("/admin/member/details/{id}")
	public String viewMemberDetails(@PathVariable Long id, Model model, HttpSession session) {
		Object isAdmin = session.getAttribute("isAdmin");
		if (isAdmin == null || !(Boolean) isAdmin)
			return "redirect:/admin/login";
		model.addAttribute("memberDetails", memberService.getMemberDetails(id));
